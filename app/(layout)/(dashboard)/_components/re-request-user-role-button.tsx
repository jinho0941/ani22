...
import { reSendUploaderApprovalRequest } from '@/app/action/user-role-request'
...
export const ReRequestUserRoleButton = () => {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    startTransition(async () => {
      ...
      const action = await reSendUploaderApprovalRequest({ title, content })
      if (!action.success) {
  ...
  return (
    <Dialog open={isOpen}>
      <DialogTrigger asChild onClick={onOpen}>
        <Button variant={'destructive'}>거절됨... 다시 신청하기</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
