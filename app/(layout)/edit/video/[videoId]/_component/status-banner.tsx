...
export const StatusBanner = ({
  ...
  const handleRequestPublish = () => {
    startTransition(async () => {
      const action = await sendApprovalVideoRequest({ videoId })
      if (action.success) {
  ...
  const handleReRequestPublish = () => {
    startTransition(async () => {
      const action = await setPendingVideoRequest({ videoId })
      if (action.success) {
        toast.success(action.message)
        router.refresh()
      } else {
  ...
  const handleCancelRequest = () => {
    startTransition(async () => {
      const action = await deleteVideoRequest(videoId)
      if (action.success) {
        toast.success(action.message)
        router.refresh()
      } else {
  ...
  const getStatusMessage = () => {
    switch (requestStatus) {
      ...
      default:
        return isCompleted
          ? '모든 항목 작성을 완료하였습니다. 비디오 배포 신청을 하십시오.'
